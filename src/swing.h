...
namespace swing {
  ...
  static int anchor, hand, grabs, rank; static float freeT, fallT; static Board board = { "swing" };
  ...
  void init() { grabs = 0; rank = -1; fallT = 0; hang(0); ringlib::reset(); cv.fillScreen(0); }
  ...
  void step(const Ctx& c) {
    ...
    if (anchor < 0 && floored && fallT <= 0) { fallT = 3; rank = board.record(grabs); dizzyT = 3; snd::note(160); buzz(150, 80); }
  ...
  void draw() {
    ...
    drawBody(); drawSparks();
    if (fallT > 0) { char s[20]; snprintf(s, sizeof s, "GRABS %d", grabs); board.draw(s, rank, ""); }
    char t[32]; snprintf(t, sizeof t, "grabs %d  best %u  %s", grabs, board.best(), face::NAME[mood]);
