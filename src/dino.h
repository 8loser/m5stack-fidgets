...
// ================= 21. 小恐龍 =================
// Chrome 斷線小恐龍:仙人掌與翼龍從右邊來,點螢幕跳、按住螢幕下半部蹲;越跑越快,每 300 分日夜切換。
// 傾斜往左壓可以稍微減速(往右加速)。撞到就結束,1.5 秒後重來;最高分保留到關機
namespace dino {
  ...
  static float y, vy, speed, dist, spawnT, endT, anim; static int score, high; static bool dead, night, duck;
  ...
  void step(const Ctx& c) {
    if (dead) { if ((endT += c.dt) > 1.5f && c.tap) init(); return; }
    duck = c.touch && c.ty > H / 2 && y <= 0;
    if (c.tap && y <= 0 && !duck) { vy = -JUMP; snd::note(600); buzz(30, 10); }
    ...
    int s = (int)(dist / 12); if (s != score) { score = s; if (score % 100 == 0) snd::note(880); if (score % 300 == 0) { night = !night; buzz(60, 30); } if (score > high) high = score; }
    ...
    for (auto& o : ob) if (o.live) {
      ...
      if (hit) { dead = true; endT = 0; snd::note(150); buzz(200, 200); }
    }
  }
  ...
  void draw() {
    ...
    if (dead) { cv.setTextDatum(middle_center); cv.setTextSize(2); cv.setTextColor(fg, bg); cv.drawString("GAME OVER", W / 2, 80); cv.setTextSize(1); cv.drawString("tap to restart", W / 2, 100); }
    char t[24]; snprintf(t, sizeof t, "HI %05d  %05d", high, score);
