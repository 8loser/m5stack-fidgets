...
namespace swing {
  ...
  void step(const Ctx& c) {
    ...
    if (anchor < 0 && freeT > 0.25f && fallT <= 0) for (int h : { LHAND, RHAND }) for (int a = 0; a < NA && anchor < 0; a++) {   // 自動抓
      float dx = p[h].x - AX[a], dy = p[h].y - AY; if (dx * dx + dy * dy < GRAB_R * GRAB_R) { anchor = a; hand = h; grabs++; if (grabs > best) best = grabs; spark(AX[a], AY, rgb(255, 230, 0)); snd::note(400 + a * 60); buzz(60, 20); }
    }
    bool floored = false; for (auto& q : p) floored |= q.y > FLOOR - 1;
    if (anchor < 0 && floored && fallT <= 0) { fallT = 1.5f; grabs = 0; dizzyT = 1.5f; snd::note(160); buzz(150, 80); }
